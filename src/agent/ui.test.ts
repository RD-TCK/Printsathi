...
vi.mock("./daemon", () => ({ agentDaemon: { getStatus: () => ({ serverUrl: "http://localhost:3000" }) } }));
...
describe("agent dashboard status", () => {
  it("renders offline hardware as offline even when the server is connected", async () => {
    ...
    const status = { isPaired: true, isConnected: true, shopName: "Shop <unsafe>", agentName: "Test", serverUrl: "http://localhost:3000", lastHeartbeat: new Date().toISOString(), printers: [{ name: "HP <M1005>", status: "offline", capabilities: { colorSupport: false } }, { name: "Microsoft Print to PDF", status: "online" }], stats: {}, recentLogs: [], currentJob: null };
    const context = vm.createContext({
      document: { getElementById: (id: string) => { if (!elements.has(id)) elements.set(id, {}); return elements.get(id); } },
    ...
    expect(elements.get("statusBadge")?.innerText).toContain("PRINTER OFFLINE");
