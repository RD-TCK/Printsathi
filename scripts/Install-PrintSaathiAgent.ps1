...
<#
.SYNOPSIS
  Printiva Desktop Agent Installer
...
.NOTES
  ...
  Both PrintivaAgent.exe and this script must be in the same folder.
#>

$ErrorActionPreference = "Stop"

# ─── Config ────────────────────────────────────────────────────────────────────
$ServiceName   = "PrintivaAgent"
$DisplayName   = "Printiva Desktop Agent"
$AgentExe      = Join-Path $PSScriptRoot "PrintivaAgent.exe"
$InstallDir    = Join-Path $env:LOCALAPPDATA "PrintivaAgent"
$NssmExe       = Join-Path $InstallDir "nssm.exe"
$InstalledExe  = Join-Path $InstallDir "PrintivaAgent.exe"
$TrayScript    = Join-Path $InstallDir "tray.ps1"
$TrayVbs       = Join-Path $InstallDir "StartTray.vbs"
$StartupLink   = Join-Path ([Environment]::GetFolderPath("Startup")) "PrintivaTray.lnk"
$DesktopLink   = Join-Path ([Environment]::GetFolderPath("Desktop")) "Printiva Dashboard.lnk"
$NssmUrl       = "https://nssm.cc/release/nssm-2.24.zip"

# ─── Banner ────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "  ╔══════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "  ║   Printiva Desktop Agent Installer            ║" -ForegroundColor Cyan
Write-Host "  ╚══════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
& $NssmExe set $ServiceName Description "Printiva Windows Desktop Agent — handles printer discovery and print job execution for your shop."
...
#   Printiva icon in the system tray. Shop owners right-click to open
#   the shop owner dashboard or the local agent status page.
#
$ServerUrl = Read-Host "  Enter your Printiva server URL (e.g. https://yourshop.printsaathi.com)"
if ([string]::IsNullOrWhiteSpace($ServerUrl)) { $ServerUrl = "http://localhost:3000" }

$trayContent = @"
# Printiva System Tray — auto-generated by installer
# Do not edit manually; re-run Install-PrintivaAgent.ps1 to update.
...
# 32x32 green printer icon (Printiva brand colour #0f766e)
`$iconBase64 = @'
...
`$timer.Add_Tick({
  try {
    ...
    if (`$r.isPaired -and `$r.isConnected) {
      `$tray.Text = "Printiva — Connected ✓ (`$(`$r.shopName))"
    } elseif (`$r.isPaired) {
      `$tray.Text = "Printiva — Paired, not connected"
    } else {
      `$tray.Text = "Printiva — Not paired yet"
    }
  } catch {
    `$tray.Text = "Printiva — Agent offline"
  }
})
`$timer.Start()
...
$startupShortcut.Description = "Printiva System Tray"
...
$desktopShortcut.Description = "Open Printiva Shop Dashboard"
...
Write-Host "  • Double-click the tray icon or use 'Printiva Dashboard'"
